...
func TestWriteFile_SizeLimit(t *testing.T) {
	...
	// 10MB + 1 byte should trigger the size limit
...
func TestExtractTarGz_BasicFiles(t *testing.T) {
	...
	for name, wantContent := range entries {
		...
		if string(got) != wantContent {
			t.Fatalf("%q content = %q, want %q", name, string(got), wantContent)
		}
	}
}

func TestExtractTarGz_CreatesDirectories(t *testing.T) {
	entries := map[string]string{
...
func TestExtractTarGz_DirectoryEntry(t *testing.T) {
	...
	got, err := os.ReadFile(filepath.Join(dst, "mydir/file.txt"))
	if err != nil {
		t.Fatalf("read file: %v", err)
	}
	if string(got) != content {
...
func TestExtractTarGz_VolumeLabel_Ignored(t *testing.T) {
	var buf bytes.Buffer
	gw := gzip.NewWriter(&buf)
	tw := tar.NewWriter(gw)
...
func TestExtractTarGz_InvalidGzip(t *testing.T) {
	archivePath := writeTempFile(t, []byte("this is not gzip"))
...
func FuzzExtractTarGz(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		// Post-condition: nothing escaped dst
		filepath.WalkDir(dst, func(path string, d os.DirEntry, err error) error {
			if err != nil {
				return nil
			}
			// Resolve symlinks (shouldn't exist, but verify)
			real, err := filepath.EvalSymlinks(path)
			if err != nil {
				return nil
			}
			if !strings.HasPrefix(real, dst) {
				t.Fatalf("file escaped destination: %q -> %q", path, real)
			}
			return nil
		})
	})
